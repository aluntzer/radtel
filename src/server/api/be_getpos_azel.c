/**
 * @file    server/api/be_getpos_azel.c
...
static int (*p_getpos_azel)(double *az, double *el);
...
int be_getpos_azel_load(GModule *mod)
{
	...
	ret = g_module_symbol(mod, "be_getpos_azel", &func);
	if(!ret)
		return -1;
	
        g_message("BACKEND: found symbol %s", __func__);

	p_getpos_azel = (typeof(p_getpos_azel)) func;
}
