/**
 * @file    acks/ack_hot_load_disable.c
...
struct packet *ack_hot_load_disable_gen(uint16_t trans_id)
{
	...
	pkt->service   = PR_HOT_LOAD_DISABLE;
...
/**
 * @brief acknowledge backend hot_load_disable command
 */
...
void ack_hot_load_disable(uint16_t trans_id)
{
	...
	pkt = ack_hot_load_disable_gen(trans_id);

	g_debug("Sending HOT LOAD DISABLE");
